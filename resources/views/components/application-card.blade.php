    ...
    <div class="">
        <div class="flex items-center justify-between">
            ...
            <div class="w-1/6 flex justify-end">
                <div class=" px-3 py-1 inline text-sm text-[#2640A1] bg-[#F3F4FA] rounded-xl capitalize">{{ $application->category }} Subject</div>
            </div>
        </div>
        ...
        <div class="md:w-64 lg:w-70 bg-white rounded-xl">
            <div class="md:p-4 lg:p-6">
                <div class="flex items-center">
                    <div class="flex items-center bg-[#F3F4FA] px-4 py-1 rounded-lg">
                        ...
                        <p class="capitalize text-sm text-[#2640A1]">
                            {{ auth()->user()->role == 'reviewer' ? 'Applicant' : 'Reviewer' }}</p>
                    </div>
                ...
                <p class="mt-3 text-center text-[#2640A1]">
                    {{ auth()->user()->role == 'reviewer' ? $application->user->name : $application->reviewer->name ?? 'NA' }}
                </p>
                <div class="flex justify-center mt-2">
                    <a href="tel:08169030947" class="inline-flex items-center bg-[#2640A1] px-3 py-1 rounded-lg">
                        <img class="mx-auto mr-2" width="10px" src="{{ asset('icons/call.png') }}" alt="profile">
                        <p class="text-white">
                            {{ auth()->user()->role == 'reviewer' ? $application->user->contact : $application->reviewer->contact ?? 'NA' }}
                        </p>
                    </a>
                </div>
            </div>
            <p class="text-center text-sm text-[#2640A1] border-t-2 py-2">
                {{ auth()->user()->role == 'reviewer' ? $application->user->email : $application->reviewer->email ?? 'NA' }}
            </p>
        </div>
        @if (auth()->user()->role != 'admin' || auth()->user()->role != 'super_admin')
            <div class="md:w-64 lg:w-70 bg-white rounded-xl md:p-4 lg:p-6 mt-5">
                <span class="px-2 py-1 text-xs text-[#2640A1] bg-[#F3F4FA] rounded-xl">Admin Comment</span>
                <div class="text-xs mt-2">{{ $application->comment ? $application->comment->comment : 'No comment' }}
                </div>
            </div>
        @else
            <div class="md:w-64 lg:w-70 bg-white rounded-xl md:p-4 lg:p-6 mt-5">
