<div class="md:w-64 lg:w-70 bg-white rounded-xl md:p-6 lg:p-8 mt-5">
    <div class="flex items-center">
        <div class="flex items-center bg-[#F3F4FA] px-4 py-1 rounded-lg">
            <img class="mx-auto mr-2" width="10px" src="{{ asset('icons/profile.png') }}" alt="profile">
            <p class="text-sm text-[#2640A1]">Applicant</p>
        </div>
        <img class="ml-auto" width="20px" src="{{ asset('icons/message.png') }}" alt="message">
    </div>
    <img class="mx-auto mt-5" width="100px" src="{{ asset('images/image.png') }}" alt="image">
    <p class="mt-3 text-center text-[#2640A1]">Funmi Akanbi</p>
    <div class="flex justify-center mt-3">
        <a href="" class="inline-flex items-center bg-[#2640A1] px-4 py-2 rounded-xl">
            <img class="mx-auto mr-2" width="10px" src="{{ asset('icons/call.png') }}" alt="profile">
            <p class="text-white">08137763345</p>
        </a>
    </div>
    <p class="text-center text-sm text-[#2640A1] mt-2">topeakanbi@gmail.com</p>
    <div class="flex justify-end mt-2">
        <p class="text-sm text-[#2640A1] bg-gray-100 px-5 py-1 rounded-xl">No task</p>
    </div>
</div>